...
#[get("")]
pub async fn get_categories(claims: crate::middleware::jwt_extractor::Claims, pool: web::Data<PgPool>) -> impl Responder {
    let rows = sqlx::query_as::<_, Category>("SELECT id, name FROM categories ORDER BY name")
...
#[post("")]
pub async fn create_category(claims: crate::middleware::jwt_extractor::Claims, pool: web::Data<PgPool>, form: web::Json<Category>) -> impl Responder {
    if !is_admin(&claims, pool.get_ref()).await {
        return HttpResponse::Forbidden().json(serde_json::json!({ "message": "Admin only" }));
    }
    let row = sqlx::query_as::<_, Category>("INSERT INTO categories (name) VALUES ($1) RETURNING id, name")
        .bind(&form.name)
...
#[patch("/{id}")]
pub async fn update_category(claims: crate::middleware::jwt_extractor::Claims, pool: web::Data<PgPool>, path: web::Path<Uuid>, form: web::Json<Category>) -> impl Responder {
    if !is_admin(&claims, pool.get_ref()).await {
        return HttpResponse::Forbidden().json(serde_json::json!({ "message": "Admin only" }));
    }
    let id = path.into_inner();
    let row = sqlx::query_as::<_, Category>("UPDATE categories SET name = $1 WHERE id = $2 RETURNING id, name")
        .bind(&form.name)
        .bind(id)
        .fetch_one(pool.get_ref())
        .await;
...
#[get("")]
pub async fn get_item_sources(claims: crate::middleware::jwt_extractor::Claims, pool: web::Data<PgPool>) -> impl Responder {
    let rows = sqlx::query_as::<_, ItemSource>("SELECT id, name FROM item_sources ORDER BY name")
        .fetch_all(pool.get_ref())
        .await;
    match rows {
        Ok(rows) => HttpResponse::Ok().json(rows),
...
#[post("")]
pub async fn create_item_source(claims: crate::middleware::jwt_extractor::Claims, pool: web::Data<PgPool>, form: web::Json<ItemSource>) -> impl Responder {
    if !is_admin(&claims, pool.get_ref()).await {
        return HttpResponse::Forbidden().json(serde_json::json!({ "message": "Admin only" }));
    }
    let row = sqlx::query_as::<_, ItemSource>("INSERT INTO item_sources (name) VALUES ($1) RETURNING id, name")
        .bind(&form.name)
        .fetch_one(pool.get_ref())
        .await;
    match row {
...
#[patch("/{id}")]
pub async fn update_item_source(claims: crate::middleware::jwt_extractor::Claims, pool: web::Data<PgPool>, path: web::Path<Uuid>, form: web::Json<ItemSource>) -> impl Responder {
    if !is_admin(&claims, pool.get_ref()).await {
        return HttpResponse::Forbidden().json(serde_json::json!({ "message": "Admin only" }));
    }
    let id = path.into_inner();
    let row = sqlx::query_as::<_, ItemSource>("UPDATE item_sources SET name = $1 WHERE id = $2 RETURNING id, name")
        .bind(&form.name)
        .bind(id)
        .fetch_one(pool.get_ref())
        .await;
...
#[get("")]
pub async fn get_conditions(claims: crate::middleware::jwt_extractor::Claims, pool: web::Data<PgPool>) -> impl Responder {
    let rows = sqlx::query_as::<_, Condition>("SELECT id, name FROM conditions ORDER BY name")
        .fetch_all(pool.get_ref())
        .await;
    match rows {
        Ok(rows) => HttpResponse::Ok().json(rows),
...
#[post("")]
pub async fn create_condition(claims: crate::middleware::jwt_extractor::Claims, pool: web::Data<PgPool>, form: web::Json<Condition>) -> impl Responder {
    if !is_admin(&claims, pool.get_ref()).await {
        return HttpResponse::Forbidden().json(serde_json::json!({ "message": "Admin only" }));
    }
    let row = sqlx::query_as::<_, Condition>("INSERT INTO conditions (name) VALUES ($1) RETURNING id, name")
        .bind(&form.name)
        .fetch_one(pool.get_ref())
        .await;
    match row {
...
#[patch("/{id}")]
pub async fn update_condition(claims: crate::middleware::jwt_extractor::Claims, pool: web::Data<PgPool>, path: web::Path<Uuid>, form: web::Json<Condition>) -> impl Responder {
    if !is_admin(&claims, pool.get_ref()).await {
        return HttpResponse::Forbidden().json(serde_json::json!({ "message": "Admin only" }));
    }
    let id = path.into_inner();
    let row = sqlx::query_as::<_, Condition>("UPDATE conditions SET name = $1 WHERE id = $2 RETURNING id, name")
        .bind(&form.name)
        .bind(id)
        .fetch_one(pool.get_ref())
        .await;
...
#[get("")]
pub async fn get_procurement_statuses(claims: crate::middleware::jwt_extractor::Claims, pool: web::Data<PgPool>) -> impl Responder {
...
#[get("")]
pub async fn get_user_roles(claims: crate::middleware::jwt_extractor::Claims, pool: web::Data<PgPool>) -> impl Responder {
